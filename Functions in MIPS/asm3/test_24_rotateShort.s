...
.data
test_ROTATE_RET_MSG:   .asciiz  " <- rotateShort() retval\n"
.text
	addi    $v0, $zero,4                        # print_str(RET_MSG)
...
j AFTER_util
	...
	# from *inside* the rotateShort() function.  So in a high-level
	# programming language, I might have passed this as a *parameter*
	# to rotateShort(), using a lambda or function pointer.  So I'm going
...
.globl utilShort
utilShort:
        addi    $a3, $zero,10   # arg4
        sw      $t0, -12($sp)
        addi    $t0, $zero,20   # arg5
        sw      $t0,  -8($sp)
        addi    $t0, $zero,30   # arg6
        sw      $t0,  -4($sp)
...
.data
util_MSG1:  .asciiz   "utilLong(): parameters: "
.text
